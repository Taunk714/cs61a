...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        aim_place = self.place
        distance = -1
        def place_search(aim_place):
            nonlocal distance
            distance += 1
            if aim_place == beehive or distance > self.max_range:
                return None
            elif aim_place.bees and distance >= self.min_range and distance <= self.max_range:
                return random_or_none(aim_place.bees)
            else:
                return place_search(aim_place.entrance)
        return place_search(aim_place)
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        choose_bee = random_or_none(self.place.bees)
        if self.digesting != 0:
            self.digesting -= 1
        elif choose_bee:
            return self.eat_bee(choose_bee)
        # END Problem 6
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        def inspire_ants(place):
            if place:
                target_ant = place.ant
                if target_ant:
                    if target_ant not in self.track_ants:
                        target_ant.damage *= 2
                        self.track_ants.append(target_ant)
                    if target_ant.is_container and target_ant.contained_ant:
                        if  target_ant.contained_ant not in self.track_ants:
                            target_ant.contained_ant.damage *= 2
                            self.track_ants.append(target_ant.contained_ant)
                return inspire_ants(place.exit)
                        
        if self.impostor:
            Insect.reduce_armor(self, self.armor)
        else:
            ThrowerAnt.action(self, colony)
            inspire_ants(self.place.exit)
        # END Problem 13
...
def make_slow(action, bee):
    ...
    def slow_action(bee,colony):
        if colony.time % 2 ==1:
            # print(1, 'slow','be slowed')
            return None
        else:
            # print(2,'slow','doing nothing')
            return action(bee,colony)
    return slow_action
    # END Problem EC
...
def make_scare(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    if bee.scared == True:
        # print(3,"can't be scared")
        return action
    else:
        bee.forward = not bee.forward
        bee.scared = True
        # print(4,"scared")
        return action
...
def apply_effect(effect, bee, duration):
    ...
    def duration_action(colony):
        ...
        if duration <= 0:
            ...
            # print(6,"effect!")
            duration -= 1
            effect(Bee.action, bee)(bee,colony)
    bee.action = duration_action
