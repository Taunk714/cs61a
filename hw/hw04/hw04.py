...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    def joint(amount, enter_password):
        if enter_password != old_password and enter_password != new_password:
            return withdraw(amount, enter_password)
        else:
            return withdraw(amount, old_password)
    init_word = withdraw(0, old_password)
    if type(init_word) == str:
        return init_word
    else:
        return joint
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0 , "The interval spans zero"
...
def multiple_references_explanation():
    return """The multiple reference problem is changed the sequence"""
...
def quadratic(x, a, b, c):
    ...
    f = []
    f.append(quad(lower_bound(x)))
    f.append(quad(upper_bound(x)))
    line = -b/(2*a)
    if line < upper_bound(x) and line > lower_bound(x):
        f.append(quad(line))
