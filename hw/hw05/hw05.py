...
def store_digits(n):
    ...
    if length == 1:
        ...
        current_digit = n // (10**(length -1))
        left_digit = n % (10**(length -1))
        return Link(current_digit,store_digits(left_digit))
...
def generate_paths(t, x):
    ...
    "*** YOUR CODE HERE ***"
    def generator(t, x, pre_path = []):
        pre_path.append(t.label)
        if t.label == x:
            if t.is_leaf():
                # print(1,pre_path)
                yield pre_path
                ...
                for branch in t.branches:
                    if x in branch:
                        for path in generator(branch,x,pre_path[:]):
                            yield path
        else:
            if t.is_leaf():
                # print(3,pre_path)
    ...
    #     if x in branch:
    #         path = [t.label]
    #         path.extend(generate_paths(branch,x))
    #     for _______________ in _________________:

            # "*** YOUR CODE HERE ***"
    # for branch in t.branches:
    #     for x in generate_paths(branch, x, pre_path):
    #         yield x
...
def deep_map(f, link):
    ...
    if link.rest:
        ...
        if isinstance(link.first, Link):
            return Link(deep_map(f, link.first), link.rest)
        else:
            return Link(f(link.first), link.rest)
